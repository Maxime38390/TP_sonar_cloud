...
public class ColorHelper {
    ...
    private static boolean shouldUseDark(int backgroundColor) {
        boolean useDark = backgroundColor == Notification.colorDefault;
        if (!useDark) {
    ...
    if (!hasForegroundColor) {
        return ensureColorsFromBackground(context, backgroundColor);
    } else {
        return ensureColorsFromForeground(backgroundColor, foregroundColor);
    }
}

private Pair<Integer, Integer> ensureColorsFromBackground(Context context, int backgroundColor) {
...
private Pair<Integer, Integer> ensureColorsFromForeground(int backgroundColor, int foregroundColor) {
    ...
    if (contrast < 4.5f) {
        return ensureColorsWithInsufficientContrast(backgroundColor, foregroundColor, backgroundLight);
    } else {
        return ensureColorsWithSufficientContrast(foregroundColor, backgroundColor, backgroundLight);
    }
}

private Pair<Integer, Integer> ensureColorsWithInsufficientContrast(int backgroundColor, int foregroundColor, boolean backgroundLight) {
    int primaryTextColor;
    int secondaryTextColor;
    if (backgroundLight) {
        secondaryTextColor = ColorHelper.findContrastColor(foregroundColor, backgroundColor, true, 4.5f);
        primaryTextColor = ColorHelper.changeColorLightness(secondaryTextColor, -LIGHTNESS_TEXT_DIFFERENCE_LIGHT);
    } else {
        secondaryTextColor = ColorHelper.findContrastColorAgainstDark(foregroundColor, backgroundColor, true, 4.5f);
        primaryTextColor = ColorHelper.changeColorLightness(secondaryTextColor, -LIGHTNESS_TEXT_DIFFERENCE_DARK);
    }
    return new Pair<>(primaryTextColor, secondaryTextColor);
}

private Pair<Integer, Integer> ensureColorsWithSufficientContrast(int foregroundColor, int backgroundColor, boolean backgroundLight) {
    int primaryTextColor = foregroundColor;
    int secondaryTextColor = ColorHelper.changeColorLightness(primaryTextColor, backgroundLight ? LIGHTNESS_TEXT_DIFFERENCE_LIGHT : LIGHTNESS_TEXT_DIFFERENCE_DARK);
    
    if (ColorHelper.calculateContrast(secondaryTextColor, backgroundColor) < 4.5f) {
        if (backgroundLight) {
            secondaryTextColor = ColorHelper.findContrastColor(secondaryTextColor, backgroundColor, true, 4.5f);
        } else {
            secondaryTextColor = ColorHelper.findContrastColorAgainstDark(secondaryTextColor, backgroundColor, true, 4.5f);
        }
        primaryTextColor = ColorHelper.changeColorLightness(secondaryTextColor, backgroundLight ? -LIGHTNESS_TEXT_DIFFERENCE_LIGHT : -LIGHTNESS_TEXT_DIFFERENCE_DARK);
    }
    return new Pair<>(primaryTextColor, secondaryTextColor);
}
